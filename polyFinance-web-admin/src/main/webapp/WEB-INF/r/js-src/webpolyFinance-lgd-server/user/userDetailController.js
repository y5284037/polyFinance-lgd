...
angular.module('polyFinance_lgd')
    .controller('userDetailCtrl', function($state,$scope, $rootScope,commonUtil,userService) {
        var vm = $scope.vm = {};
        vm.id = $state.params.id;

        if (vm.id) {
            userService.getModule(vm.id).then(function(res) {
                if (res.data.code == 0) {
    ...
    vm.add = function() {
        ...
        userService.add(vm.params).then(function(res) {
            if (res.data.code === 0) {
    ...
    vm.update = function() {
        ...
        userService.update(id, vm.params).then(function(res) {
            if (res.data.code === 0) {
